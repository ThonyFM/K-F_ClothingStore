...
BEGIN
    ...
    BEGIN
        RAISERROR('No se puede eliminar la direcci�n porque est� en uso.', 16, 1);
        RETURN;
    END
...
BEGIN
    ...
    BEGIN
        RAISERROR('No se puede eliminar la persona porque est� asociada a un cliente o empleado.', 16, 1);
        RETURN;
    END
...
CREATE PROCEDURE sp_GetPersonaById
    @ID INT
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE PROCEDURE sp_GetDescuentoById
    @ID INT
AS
BEGIN
    SET NOCOUNT ON;
    SELECT * FROM Descuento WHERE ID = @ID;
END
GO
